...
class GCNNoise(BaseGraphClassifier):
    ...
    def __init__(self, input_dim: int, number_of_labels: int, hidden_dim: int = 16):
        super(GCNNoise, self).__init__(input_dim, number_of_labels)
    ...
    def forward(self, graph: dgl.DGLGraph, edge_weight=None) -> torch.tensor:
        ...
        x = torch.relu(self.graph_conv3(graph, x, edge_weight=edge_weight)) + noise
